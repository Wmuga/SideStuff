...
void bmp_writer::write_pixel(size_t x, size_t y, tagRGBQUAD _color,double alpha) {

    size_t sizeofcolor = this->bih.biBitCount / 8;
    size_t y1 = this->bih.biHeight - y - 1;
    size_t line = this->bih.biWidth;
    if (x < bih.biWidth && y1 < bih.biHeight) {
        fseek(this->img,
              this->bfh.bfOffBits + sizeofcolor * x + y1 * (line * sizeofcolor + (((4 - sizeofcolor) * line) % 4)),
              SEEK_SET);
        tagRGBQUAD current_color{};
        fread(&current_color, sizeofcolor, 1, this->img);
        fseek(this->img,
              this->bfh.bfOffBits + sizeofcolor * x + y1 * (line * sizeofcolor + (((4 - sizeofcolor) * line) % 4)),
              SEEK_SET);
        tagRGBQUAD new_color = (current_color*(1-alpha)+_color*alpha);
        fwrite(&new_color, sizeofcolor, 1, this->img);
    }
}

void bmp_writer::write_line(size_t x1, size_t y1, size_t x2, size_t y2, tagRGBQUAD _color,double alpha) {
    ...
    {
        write_pixel(x,y,_color,alpha);
        if (step_x<1.0){
            write_pixel(x-1,y,_color,alpha*0.7);
            write_pixel(x+1,y,_color,alpha*0.7);
        }
        else{
            write_pixel(x,y-1,_color,alpha*0.7);
            write_pixel(x,y+1,_color,alpha*0.7);
        }
        x=ceil(x+step_x);
        y=ceil(y+step_y);
...
void bmp_writer::write_digit(size_t x, size_t y, int digit, tagRGBQUAD _color,double alpha) {
    bool* digit_array;
    switch (digit)
    {
        default:
    ...
    for (size_t _y=0;_y<5;_y++)
    {
        ...
        {
            if(*digit_array) write_pixel(x+_x,y+_y,_color,alpha);
            digit_array++;
        }
    }
}

void bmp_writer::write_circle(size_t x, size_t y, size_t r, tagRGBQUAD color , bool is_hollow,double alpha) {
...
void bmp_writer::write_rectangle(size_t x, size_t y , size_t size_x, size_t size_y, size_t angle, tagRGBQUAD _color,double alpha) {
    ...
    for (;y<=y2;y++)
    {
        write_line(x,y,x1,y1,_color,alpha);
        y1++;
        x+=step_x;
        x1+=step_x;
    }
...
tagRGBQUAD operator+(const tagRGBQUAD & l, const tagRGBQUAD & r) {
    tagRGBQUAD new_color{};
    new_color.rgbBlue = l.rgbBlue + r.rgbBlue;
    new_color.rgbRed = l.rgbRed + r.rgbRed;
    new_color.rgbGreen = l.rgbGreen + r.rgbGreen;
    new_color.rgbReserved = l.rgbReserved + r.rgbReserved;
    return new_color;
}

tagRGBQUAD operator*(const tagRGBQUAD & source,double size) {
    tagRGBQUAD new_color{};
    new_color.rgbBlue = source.rgbBlue*size;
    new_color.rgbRed = source.rgbRed*size;
    new_color.rgbGreen = source.rgbGreen*size;
    new_color.rgbReserved = source.rgbReserved*size;
    return new_color;
}
